...
int openat_257_policy(pid_t pid, char *arg0, char *arg1, char *arg2, char *arg3) {
    ...
    if (!arg0 || !arg1 || !arg2 || !arg3) {
        fprintf(stderr, "[openat] Invalid arguments\n");
        return 0;
    }

    int dirfd = atoi(arg0);
    unsigned long pathname_ptr = strtoul(arg1, NULL, 10);
    int flags = atoi(arg2);
    int mode  = atoi(arg3);
    ...
    if (!is_safe_ptr(pathname_ptr)) {
        fprintf(stderr, "[openat] Unsafe pathname pointer: 0x%lx\n", pathname_ptr);
        return 0;
    }

    /* read pathname from child memory */
    if (read_child_string(pid, pathname_ptr, path, sizeof(path)) < 0) {
        fprintf(stderr, "[openat] Cannot read pathname\n");
        return 0;
    }

    /* policy: path */
    if (!is_safe_path(path)) {
        fprintf(stderr, "[openat] Dangerous path: %s\n", path);
        return 0;
    }

    /* policy: flags */
    if (is_dangerous_flags(flags)) {
        fprintf(stderr, "[openat] Dangerous flags: 0x%x\n", flags);
        return 0;
    }
    ...
    if (flags & O_CREAT) {
        if (is_dangerous_mode(mode)) {
            fprintf(stderr, "[openat] Dangerous file mode: %o\n", mode);
            return 0;
        }
    }
